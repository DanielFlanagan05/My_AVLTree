...
class AVLTree:
    ...
    ###
    ### My work begins here. Above is from professor's template for the project.
    ###
    ...
    def levelorder(self, root: Node) -> Generator[Node, None, None]:
        ...
        # This works by placing the root into the queue, then both of its child nodes. Because the queue puts the nodes
        # in left to right order for each level, yielding each node in this order results in level order
        while not q.empty():
            current_node = q.get()
            yield current_node
    ...
    ####################################################################################################
###
### Function below was provided by my professor
###
...
class Table:
    ...
    ###
    ### My work on Table class begins here
    ###
    def insert_rows(self, values: List[List[str]], attributes: List[str]) -> None:
        ...
        for row in values:
            reordered_row = []
            for attribute in self.header_row[1:]:
                index = attributes.index(attribute)
                value = row[index]
                reordered_row.append(value)

            self.data_tree.insert(self.data_tree.origin, self.insertion_index, reordered_row)
    ...
    def remove_rows(self, indices: List[str]) -> None:
        ...
        for index_str in indices:
            index = int(index_str)
            self.data_tree.remove(self.data_tree.origin, index)
    ...
    def show_latest(self) -> List[List[str]]:
        ...
        """
        if self.data_tree.origin is None:
            return [self.header_row]

        latest_node = self.data_tree.max(self.data_tree.origin)

        latest_row = [latest_node.value] + latest_node.data

        return [self.header_row, latest_row]
    ...
    def show_oldest(self) -> List[List[str]]:
        ...
        oldest_node = self.data_tree.min(self.data_tree.origin)

        oldest_row = [oldest_node.value] + oldest_node.data

        return [self.header_row, oldest_row]
    ...
    def show_everything(self) -> List[List[str]]:
        ...
        for node in self.data_tree.inorder(self.data_tree.origin):
            row = [node.value] + node.data
